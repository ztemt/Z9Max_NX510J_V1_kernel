		...
		if (dev_map[idx].stream_ref_count == 0) {
			...
			if (ds2_dap_params_states.dap_bypass_type ==
				...
				ret = msm_ds2_dap_alloc_and_store_cal_data(idx,
						       ADM_PATH_PLAYBACK, 0);
				if (ret < 0) {
					pr_err("%s: Failed to alloc and store "
					       "cal data for idx %d, device %d, "
					       "copp_idx %d",
					       __func__,
				...
				ret = msm_ds2_dap_init_modules_in_topology(
							idx);
				if (ret < 0) {
					pr_err("%s: Failed to init modules in "
					       "topolofy for idx %d, device %d, copp_idx %d\n",
					       __func__, idx, dev_map[idx].device_id,
					       dev_map[idx].copp_idx);
					dev_map[idx].active = false;
					dev_map[idx].copp_idx = -1;
					goto end;
