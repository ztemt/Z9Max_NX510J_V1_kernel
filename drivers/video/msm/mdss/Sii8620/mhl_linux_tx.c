...
struct device_attribute driver_attribs[] = {
	...
	__ATTR(SYS_ATTR_NAME_SPAD		,0664,show_scratch_pad	, send_scratch_pad),
	__ATTR(SYS_ATTR_NAME_I2C_REGS	,0664,show_i2c			, set_i2c),
	__ATTR(SYS_ATTR_NAME_DEBUG_LEVEL,0664,get_debug_level	, set_debug_level),
	__ATTR(SYS_ATTR_NAME_REG_DEBUG_LEVEL,0664,get_reg_debug_level	, set_reg_debug_level),
	__ATTR(SYS_ATTR_NAME_GPIO_INDEX ,0664,get_gpio_index	, set_gpio_index),
	__ATTR(SYS_ATTR_NAME_GPIO_VALUE ,0664,get_gpio_level	, set_gpio_level),
...
#ifdef DEBUG
	__ATTR(SYS_ATTR_NAME_TX_POWER	,0220,NULL				, set_tx_power),
	__ATTR(SYS_ATTR_NAME_STARK_CTL	,0220,NULL				, set_stark_ctl),
#endif
#ifdef BIST_INITIATOR
	__ATTR(SYS_ATTR_NAME_BIST	,0220,NULL				, set_bist),
#endif
	__ATTR_NULL
};
...
static struct	device_attribute rap_in_status_attr 	=__ATTR(SYS_ATTR_NAME_RAP_IN_STATUS	,0220,NULL					,set_rap_in_status);
static struct	device_attribute rap_out_attr 			=__ATTR(SYS_ATTR_NAME_RAP_OUT		,0664,show_rap_out			,send_rap_out);
static struct	device_attribute rap_out_status_attr	=__ATTR(SYS_ATTR_NAME_RAP_OUT_STATUS,0444,show_rap_out_status	,NULL);
static struct	device_attribute rap_input_dev			=__ATTR(SYS_ATTR_NAME_RAP_INPUT_DEV	,0664,show_rap_input_dev	,set_rap_input_dev);
...
static struct	device_attribute rcp_in_status_attr 	=__ATTR(SYS_ATTR_NAME_RCP_IN_STATUS ,0220,NULL				,send_rcp_in_status);
static struct	device_attribute rcp_out_attr 			=__ATTR(SYS_ATTR_NAME_RCP_OUT		,0664,show_rcp_out		,send_rcp_out)   ;
static struct	device_attribute rcp_out_status_attr	=__ATTR(SYS_ATTR_NAME_RCP_OUT_STATUS,0444,show_rcp_out_status,NULL)             ;
static struct	device_attribute rcp_input_dev			=__ATTR(SYS_ATTR_NAME_RCP_INPUT_DEV ,0664,show_rcp_input_dev,set_rcp_input_dev) ;
...
static struct	device_attribute ucp_in_status_attr 	=__ATTR(SYS_ATTR_NAME_UCP_IN_STATUS ,0220,NULL				,send_ucp_in_status);
static struct	device_attribute ucp_out_attr 			=__ATTR(SYS_ATTR_NAME_UCP_OUT		,0664,show_ucp_out		,send_ucp_out)   ;
static struct	device_attribute ucp_out_status_attr	=__ATTR(SYS_ATTR_NAME_UCP_OUT_STATUS,0444,show_ucp_out_status,NULL)             ;
static struct	device_attribute ucp_input_dev			=__ATTR(SYS_ATTR_NAME_UCP_INPUT_DEV ,0664,show_ucp_input_dev,set_ucp_input_dev) ;
...
static struct	device_attribute attr_devcap_local_offset	=__ATTR(SYS_ATTR_NAME_DEVCAP_LOCAL_OFFSET	,0664,show_devcap_local_offset	,set_devcap_local_offset	);
static struct	device_attribute attr_devcap_local			=__ATTR(SYS_ATTR_NAME_DEVCAP_LOCAL			,0664,show_devcap_local			,set_devcap_local			);
static struct	device_attribute attr_devcap_remote_offset	=__ATTR(SYS_ATTR_NAME_DEVCAP_REMOTE_OFFSET	,0664,show_devcap_remote_offset	,set_devcap_remote_offset	);
...
static struct	device_attribute attr_hdcp_force_content_type	=__ATTR(SYS_ATTR_NAME_HDCP_CONTENT_TYPE,0664,show_hdcp_force_content_type	,set_hdcp_force_content_type);
...
//#if 0
			...
			if(dev_context->intr_info.flags & DRV_INTR_FLAG_DISCONNECT) {
				mhl_tx_stop_timer(dev_context,dev_context->hpd_timer);//addd by gary
...
int mhl_handle_power_change_request(struct device *parent_dev, bool power_up)
{
	...
	if (power_up)
		status = dev_context->drv_info->mhl_device_initialize(
					(struct drv_hw_context *)(&dev_context->drv_context));
	else
...
struct mhl_dev_context dev_context_bk;
...
static int EventThread(void *data)
{
	...
	while (true)
	{
		...
		msleep(30);
		//SiiMhlTxDeviceIsr();
		if(1)mhl_irq_handler(gpio_to_irq(gpio_int),(void *)(&dev_context_bk));
	}
	return 0;
}
			     ...
			     void (*usb_notify_cb)(void *, int), void *ctx)
{
	...
	//added by zhang yue on 2014-07-17 for debugging power consume start
	
     mhl_power_on(dev_context->mhl_dev);
	//disable_irq(dev_context->drv_info->irq);
	//si_mhl_tx_initialize(dev_context);
	//added by zhang yue on 2014-07-17 for debugging power consume end
	dev_context->drv_info->mhl_device_initialize(
					(struct drv_hw_context *)(&dev_context->drv_context));
	//change switch to MHL 
	//set_pin(X02_USB_SW_CTRL, 1);
	//liping delte 	gpio_direction_output(GPIO_BB_ID_SEL, 1);
	//enable_irq(dev_context->drv_info->irq);	
	//liping delte gpio_set_value(GPIO_BB_POWER_1V0, 1);
	pr_err(":gpio_idsel level=%d\n", gpio_get_value(gpio_idsel));
	//gpio_direction_output(gpio_idsel, 1);
	gpio_set_value(gpio_idsel, 1);
	pr_err("gpio_idsel level=%d@@@\n", gpio_get_value(gpio_idsel));

	if(mhl_sii_wait_for_rgnd(dev_context)&&0) {
		pr_err("%s: discovery timeout\n", __func__);

		//mhl_sii_config(mhl_ctrl, false);
		//change switch to usb
		//set_pin(X02_USB_SW_CTRL, 0);
		//gpio_direction_output(gpio_idsel, 0);//comment by garyyuan 20141115
		
		mhl_power_stop(dev_context->mhl_dev);
...
void mhl_tx_start_timer_t(void)
{
   	MHL_TX_DBG_WARN("mhl_tx_start_timer_t......\n");
	mhl_tx_start_timer((void *)(&dev_context_bk), (void *)(&dev_context_bk)->hpd_timer,2000);
}
