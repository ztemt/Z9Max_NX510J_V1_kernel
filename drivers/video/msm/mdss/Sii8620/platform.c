...
/* System GPIOs should into DTSI files,this is devices tree node*/
...
static struct regulator *vdd_power=NULL;
...
bool	input_dev_rcp	= 1;	// TODO: Keep default enabled when Feature complete
...
static void toggle_reset_n(int reset_period)
{
	MHL_TX_DBG_INFO("Toggle RESET_N pin\n");
	//gpio_set_value(GPIO_BB_RESET, 0);
	gpio_set_value(gpio_reset, 0);
	pr_err("gpio_reset=%d\n", gpio_get_value(gpio_reset));
	msleep(reset_period);		// Without this, we see a 500ns reset pulse. Enforce 1ms.
...
{
	return (gpio_get_value(gpio_int) ? 0 : 1);
}
...
void mhl_tx_vbus_control(enum vbus_power_state power_state)
{
	...
	dev_err(dev_context->mhl_dev,
					"%s:new power_state %d received!VBUS_power_state=%d\n",
...
static struct gpio starter_kit_control_gpios[] =
{
	 ...
	 */
	{gpio_int,		GPIOF_IN,				"MHL_intr"},
	{gpio_reset,		GPIOF_OUT_INIT_HIGH,	"MHL_reset"},
	{gpio_wake,   GPIOF_OUT_INIT_HIGH,       "MHL_fwwake"},
...
#endif
#endif
	{gpio_idsel,GPIOF_OUT_INIT_HIGH,"MHL_select"},
};
#endif
...
static int sii_init_gpio(struct device *dev)
{
	...
	gpio_idsel= of_get_named_gpio(of_node, "sii8620-idsel-gpio", 0);
	pr_err("[sii8620]%s gpio_idsel=%d\n", __func__, gpio_idsel);
	if(!gpio_is_valid(gpio_idsel)){
		printk("[sii8620]%s : gpio idsel invalid :%d\n", __func__,
				gpio_idsel);
		return -EBUSY;
	}
	ret = gpio_request(gpio_idsel, "sii8620_sel");
	if (ret) {
		printk("[sii8620]%s : failed to requset sel gpio %d\n", __func__,
				gpio_idsel);
		goto err1;
	}
	ret = gpio_direction_output(gpio_idsel,0);
	if(ret < 0){
	...
	if(vdd_power==NULL)
		vdd_power=regulator_get(dev, "vdd_power");
	//mhl_power_on(dev);

	//fw_wake
	gpio_wake= of_get_named_gpio(of_node, "sii8620-wake-gpio", 0);
	if(!gpio_is_valid(gpio_wake)){
		printk("[sii8620]%s : gpio wake:%d\n", __func__,
				gpio_wake);
		return -EBUSY;
	}
	ret = gpio_request(gpio_wake, "sii8620_fw_wake");
	if (ret) {
		printk("[sii8620]%s : failed to wake gpio %d,ret=%d\n", __func__,
				gpio_wake,ret);
		gpio_free(gpio_wake);
		goto err2;
	}
	//msleep(200);//200ms
	ret = gpio_direction_output(gpio_wake,1);
	if(ret < 0){
		printk("[sii8620]%s : wake gpio output 1 error :%d\n", __func__,
				gpio_wake);
		return -EBUSY;
	}
	...
	if (ret) {
			pr_err("[sii8620]%s : failed to request int gpio %d\n", __func__,gpio_int);
			goto err3;
	}
	
	printk("[sii8620]==========%s:line%d,gpio_reset=%d.\n",__func__,__LINE__, gpio_int);
	
	gpio_direction_input(gpio_int);
	pr_err(" gpio_int=%d\n", gpio_get_value(gpio_int));
	printk("[sii8620]==========%s:line%d,gpio_reset=%d.\n",__func__,__LINE__, gpio_int);
					      ...
					      const struct i2c_device_id *id)
{
	...
	if (!adapter || !i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		pr_err("[ERROR] %s() i2c no function\n", __func__);
		ret =  -EIO;
		goto done;
	}
	...
	if (NULL == i2c_mem.block_tx_buffers) {
		pr_err("[ERROR] %s() NULL == i2c_mem.block_tx_buffers\n", __func__);
		ret = -ENOMEM;
		goto done;
	}
	...
	if (ret >= 0) 
		{
		drv_info.irq = gpio_to_irq(gpio_int);
		printk("[sii8620]%s: sii_init_gpio.irq = %d\n", __func__,drv_info.irq);

		/*liping debug
		gpio_set_value(GPIO_BB_POWER_1V0, 0);
		gpio_set_value(gpio_wake, 0);
		*/
		//mhl_power_stop(&client->dev);
		//gpio_set_value(pm_bb_power1v0, 0);
...
int si_8620_pm_resume(struct device *dev)
{
	...
	printk(KERN_ERR "%s will switch ID and turn on power for 8620\n", __func__);
...
static struct dev_pm_ops si_8620_tx_pm_ops = {
	.runtime_suspend = NULL,//si_8620_pm_suspend,
	.runtime_resume = NULL,//si_8620_pm_resume,
};
...
static struct i2c_board_info __initdata si_8620_i2c_boardinfo[] = {
	{
		...
		.irq = 344,//gpio_to_irq(GPIO_MHL_INT),//OMAP_GPIO_IRQ(GPIO_MHL_INT),ztemt344,//343,//
	}
};
#endif
