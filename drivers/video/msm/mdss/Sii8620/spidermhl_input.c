...
static struct {
	...
	#ifdef MDT_SUPPORT
	unsigned char    bad_mouse;				//allow a workaround for old mice
	#endif
} spidermouse_driver;
...
static int spidermouse_handle_event(const struct spider_event *event)
{
	...
	if (!dev || !dev->users) {
		#ifdef MDT_SUPPORT								// Simulate a hot-plug message
			spidermouse_enable(dev_type);						//     in case one isn't generated.
		#endif
		pr_err("spidermouse: %s: no dev or no user!\n", __func__);
		return -ENODEV;
	}

	#ifdef MDT_SUPPORT									// Ignore touch pad input if mouse
	//if ((index == 0) && (spidermouse_get_drvdata(1) != NULL))				//     is available.
	//	return 0;									// Disable this feature since override exists

	if (!index)
		value = (char *)&event->pmouse_data[0];
	else {
		if ((spidermouse_driver.bad_mouse == 1) && 					// Old mouse workaround
		   ((event->umouse_data[1] > 4) || (event->umouse_data[1] < 0)))		//    See probe function for details.
		     spidermouse_driver.bad_mouse = 0;
		value = (char *)&event->umouse_data[spidermouse_driver.bad_mouse];
	}	
	#else	
	value = !index ?
	...
	printk(KERN_INFO "spidermouse: %s: %#02x %d %d %d\n", __func__,
	...
	#ifndef MDT_SUPPORT									// Bug found in Spider; usb mouse is
	input_report_rel(dev, REL_WHEEL, dz);							//     offset by 1 byte
	#endif

	input_report_key(dev, BTN_LEFT,  new_buttons & 1);
	input_report_key(dev, BTN_RIGHT, new_buttons & 2);
	#ifndef MDT_SUPPORT									// This is not the standard way to 
	input_report_key(dev, BTN_BACK,  new_buttons & 4);					//     interpret the middle button
	#endif
...
#ifdef MDT_SUPPORT
void spidermouse_enable(int dev_type)
#else
...
#ifdef MDT_SUPPORT
int spidermouse_disable(int dev_type)
#else
...
static void spidermouse_control(struct work_struct *work)
{
	...
	if (SM_DEV_STATE_CONNECTED & new_state)
		...
		#ifdef MDT_SUPPORT		// old mouse workaround support
		spidermouse_driver.bad_mouse = 1;
		#endif
...
static int __init spidermouse_probe(void)
{
	...
	#ifdef MDT_SUPPORT			// Enable workaround until proven otherwise. Spider has been found to send
	spidermouse_driver.bad_mouse = 1;	// button byte at offest 1 for old mice. If offset 1 carries X change, 
	#endif					//    the value will be greater than allowed buttons or negative.
						//    In the case offset 1 has X change, will clear this state.
	dbg_leave();
