...
int si_mhl_tx_reserve_resources(struct mhl_dev_context *dev_context)
{
	...
	ret = mhl_tx_create_timer(dev_context, hpd_timer_callback,
							dev_context, &dev_context->hpd_timer);
		if (ret != 0) {
...
int si_mhl_tx_initialize(struct mhl_dev_context *dev_context)
{
	...
	//return dev_context->drv_info->mhl_device_initialize(	(struct drv_hw_context *)(&dev_context->drv_context));
	return 0;
}
...
static void hpd_timer_callback(void *callback_param)
{
	...
	Judge_SCDT((struct drv_hw_context *)(&dev_context->drv_context));
...
void si_mhl_tx_process_events(struct mhl_dev_context *dev_context)
{
	...
	if (dev_context->mhl_connection_event) {
		...
		if (MHL_TX_EVENT_DISCONNECTION == dev_context->mhl_connected) {
			...
			si_mhl_tx_reset_states(dev_context);
			//added by zhang yue on 2014-07-21 for debugging power consumtion start
			si_mhl_tx_shutdown(dev_context);
			//added by zhang yue on 2014-07-21 for debugging power consumtion end
		}
			...
			if (MHL_RAP_CBUS_MODE_DOWN ==
				...
				/*MHL_TX_DBG_ERR
				    ("Got RAPK{%s} for RAP{CBUS_MODE_DOWN}\n",
				     rapk_error_code_string[dev_context->msc_msg_data & 0x03]);*/
				   //  MHL_TX_DBG_ERR ("Got RAPK{%d} for RAP{CBUS_MODE_DOWN}\n",  [dev_context->msc_msg_data & 0x03]);
				si_mhl_tx_drv_switch_cbus_mode(
					(struct drv_hw_context *)
					&dev_context->drv_context,
					CM_oCBUS_PEER_IS_MHL3);
				...
				struct cbus_req *req, uint8_t data1)
{
	...
	if (MHL_DEV_CATEGORY_POW_BIT & devcap_changes.mdc.
		...
		if (param) {
			...
			power_supply_set_current_limit(&(qualcomm_p_data->mhl_psy), plim_table[index]);
			power_supply_changed(&(qualcomm_p_data->mhl_psy));
			MHLSinkProvidePower(true);
		} else {
