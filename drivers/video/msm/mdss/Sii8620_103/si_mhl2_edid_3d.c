...
uint8_t EDID_block_data_t [2*EDID_BLOCK_SIZE];
...
static void si_mhl_tx_prune_edid(struct edid_3d_data_t *mhl_edid_3d_data)
{
	 ...
	 */
	 printk(KERN_ERR "memcpy(&EDID_block_data_t,&mhl_edid_3d_data->EDID_block_data,2*EDID_BLOCK_SIZE);!!!!!!!\n\n");
	memcpy(&EDID_block_data_t,&mhl_edid_3d_data->EDID_block_data,2*EDID_BLOCK_SIZE);

	DUMP_EDID_BLOCK(0,EDID_block_data_t,2*EDID_BLOCK_SIZE);
	...
	if (mhl_edid_3d_data->parse_data.HDMI_sink) {
		...
		if (mhl_peer_version >= 0x20) {
			...
			CLR_3D_FLAG(mhl_edid_3d_data,
				FLAGS_BURST_3D_DTD_VESA_DONE);
#if 0//ndef EDID_PASSTHROUGH //(
		printk(KERN_ERR "memcpy(&EDID_block_data_t,&mhl_edid_3d_data->EDID_block_data,2*EDID_BLOCK_SIZE);\n\n");
		memcpy(&EDID_block_data_t,&mhl_edid_3d_data->EDID_block_data,2*EDID_BLOCK_SIZE);
		DUMP_EDID_BLOCK(0, (uint8_t *) EDID_block_data_t, 2*EDID_BLOCK_SIZE);
