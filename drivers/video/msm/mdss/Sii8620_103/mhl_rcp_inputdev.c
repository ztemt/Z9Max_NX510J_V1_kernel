...
/* 0x00=Select				*/	 {0,0,0,{KEY_SELECT			,0},(MHL_DEV_LD_GUI)		}
...
{
	int	status = -EINVAL;
	MHL_TX_DBG_ERR("received RCP key:0x%x\n",rcp_keycode);
	...
	if (dev_context->rcp_input_dev){
		if (rcp_keycode < ARRAY_SIZE(rcp_def_keymap) &&
			...
			MHL_TX_DBG_ERR("input_report_key key:0x%x\n",rcp_keycode);

			input_report_key(dev_context->rcp_input_dev, rcp_def_keymap[rcp_keycode&0x7F], 1);
			input_report_key(dev_context->rcp_input_dev, rcp_def_keymap[rcp_keycode&0x7F], 0);
...
static int rcp_trigger_key_action(struct mhl_dev_context *dev_context,uint8_t index,bool press_release)
{
	...
	if (dev_context->rcp_input_dev){
		MHL_TX_DBG_ERR("rcp_trigger_key_action: received RCP key:0x%x\n",index);
		input_report_key(dev_context->rcp_input_dev, index, press_release);
		input_sync(dev_context->rcp_input_dev);
		status = 0;
	}
	return status;
...
static int handle_rcp_event(struct mhl_dev_context *dev_context,
							 uint8_t current_key,uint8_t prev_key,rcp_event_e event)
{
int status = 0;
							 ...
							 uint8_t rcp_keycode)
{
	...
	*/
	...
	MHL_TX_DBG_ERR("received RCP key:0x%x,rcp_def_keymap[index] =0x%x\n",rcp_keycode,rcp_def_keymap[index] );
	...
	//	rcp_def_keymap[index] != KEY_RESERVED)
		...
		status = handle_rcp_event(dev_context, rcp_def_keymap[index] ,rcp_def_keymap[rcp_current_key&0x7F],event);
	}
...
//	rcp_input_dev->id.product = 0x8620;
	...
	if (ret != 0){
		...
		if (ret != 0){
			MHL_TX_DBG_ERR("failed to create timer_T_hold_maintain!\n");
		}else{
