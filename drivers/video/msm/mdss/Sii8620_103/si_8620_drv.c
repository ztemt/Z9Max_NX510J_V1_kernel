...
void dump_all_registers(struct drv_hw_context *hw_context)
{
	...
	for (i = 0; i < 1; ++i) {
		...
		for (j = 0; j < 256; ++j) {
			...
			dummy = mhl_tx_read_reg(hw_context, TX_PAGE_3|j);
			printk(KERN_ERR "pages[3]:0x%2x:  0x%2x    " ,j,dummy);
			if(j%16==0)
				printk(KERN_ERR "\n");
		}
		printk(KERN_ERR "\n\n\n");
		for (j = 0; j < 256; ++j) {
			int dummy;
			//dummy = mhl_tx_read_reg(hw_context, pages[i]+j);
			dummy = mhl_tx_read_reg(hw_context, TX_PAGE_7|j);
			printk(KERN_ERR "pages[7]:0x%2x:  0x%2x    ",j,dummy);
			if(j%16==0)
				printk(KERN_ERR "\n");
		}
	}
...
static int coc_isr(struct drv_hw_context *hw_context, uint8_t coc_int_status)
{
	...
	if (BIT_COC_PLL_LOCK_STATUS_CHANGE & coc_int_status){
		...
		printk(KERN_ERR "COC PLL lock status change..\n\n");
		}
	...
	if (si_mhl_tx_drv_connection_is_mhl3(dev_context)) {
		...
		if (BIT_COC_CALIBRATION_DONE & coc_int_status) {
			...
			printk(KERN_ERR "Calibration done.\n\n");
	...
	enum cbus_mode_e mode_sel)
{
	...
	if (hw_context->cbus_mode < CM_oCBUS_PEER_VERSION_PENDING)
		return -1;
	printk(KERN_ERR "Switch to mode_sel:%d...\n\n",mode_sel);	
	switch (mode_sel) {
	...
	case CM_oCBUS_PEER_IS_MHL1_2:
		...
		printk(KERN_ERR "Switch to MHL1/2 oCBUS mode...\n\n");
...
static int set_hdmi_params(struct mhl_dev_context *dev_context)
{
	...
	hw_context->outgoingAviPayLoad =
	    hw_context->current_avi_info_frame.payLoad.hwPayLoad;
	MHL_TX_DBG_ERR ("timing_info_basis=0x%d!!! ",timing_info_basis);
...
/* end BZ 32674 ) */
	...
	uint8_t i;
	for(i=0;i<10;i++)
		printk(KERN_ERR "hw_context->outgoingAviPayLoad.ifData[%d]=0x%x.\n",i,hw_context->outgoingAviPayLoad.ifData[i]);
	
	for(j=0;j<=0xFF;j++){
		printk(KERN_ERR "[0x%x]=0x%x   ",j,mhl_tx_read_reg(hw_context, TX_PAGE_3 | j));
		if(j%16==0)
			printk(KERN_ERR "\n");
		}
		}*/	
	...
	uint8_t *edid, uint16_t length)
{
	...
	//printk(KERN_ERR "si_mhl_tx_drv_set_upstream_edid() called\n\n");
...
static int hdcp2_isr(struct drv_hw_context *hw_context, uint8_t intr_status)
{
	...
	if (intr_status & BIT_HDCP2_INTR_AUTH_DONE) {
		...
		//MHL_TX_DBG_INFO("@@@@@@@@@@@@change MHL3 mode TMDS setting@@@@@@@@@@@@@@@@@@@@@@@@@@@@.\n");
		/*mhl_tx_write_reg(hw_context, REG_MHL_DP_CTL1, 0xA2);
			...
			mhl_tx_write_reg(hw_context, REG_MHL_DP_CTL5, 0x01);
			mhl_tx_write_reg(hw_context, REG_MHL_DP_CTL6, 0x02);*/
			//mhl_tx_write_reg(hw_context, REG_MHL_DP_CTL7, 0x04);
			//mhl_tx_write_reg(hw_context, REG_COC_CTLC, 0xFF);
	
		//dump_all_registers(hw_context);
...
int Judge_SCDT(struct drv_hw_context *hw_context)
{
	...
	/* Measure the HTOTAL and VTOTAL and look them up in a table */
	h_total = (((uint16_t)mhl_tx_read_reg(hw_context, REG_H_RESH)) <<8) |
				(uint16_t)mhl_tx_read_reg(hw_context,, REG_H_RESL);
	v_total = (((uint16_t)mhl_tx_read_reg(hw_context, REG_V_RESH)) <<8) |
				(uint16_t)mhl_tx_read_reg(hw_context,, REG_V_RESL);
...
static void switch_to_idle(struct drv_hw_context *hw_context,
			 bool do_interrupt_clear)
{
	printk(KERN_ERR "switch_to_idle\n\n");
	...
	mhl_tx_write_reg(hw_context, REG_MHL_CBUS_CTL0,
			 ( VAL_MHL_CBUS_CTL0_CBUS_DRV_SEL_STRONGEST|//VAL_MHL_CBUS_CTL0_CBUS_DRV_SEL_STRONG
...
static int int_4_isr(struct drv_hw_context *hw_context, uint8_t int_4_status)
{
	...
	if ((BIT_CBUS_MHL12_DISCON_INT & int_4_status) ||
		...
		printk(KERN_ERR "MHL disconnected, int_4_status=0x%x\n\n",int_4_status);
		power_supply_set_online(&(qualcomm_p_data->mhl_psy), false);
		...
		/*if(wake_lock_active(&dev_context->mhl_on_wake_lock)){
			...
			printk(KERN_ERR "wake_unlock(&dev_context->mhl_on_wake_lock);");
			}*/
		/* For proper MHL discovery and tolerance of impedance
		 * measurement, set 5E3[7:4]=0x01 - this turns off the
		 * CBUS pull up for discovery states and 20K for IDLE
		...
		if (VAL_RGND_1K == disc_stat2) {
			...
			printk(KERN_ERR "Cable impedance = 1k (MHL Device)\n\n");
			MHL_TX_DBG_WARN("Cable impedance = 1k (MHL Device)\n");
			//added by zhang yue for qualcomm platform start
			MHL_TX_DBG_WARN("qualcomm_p_data->hdmi_mhl_ops->set_upstream_hpd start\n");
			//
			...
			if(qualcomm_p_data->hdmi_mhl_ops && qualcomm_p_data->hdmi_mhl_ops->set_upstream_hpd){
				...
				MHL_TX_DBG_WARN("qualcomm_p_data->hdmi_mhl_ops->set_upstream_hpd end\n");
			}
			#if 1
			power_supply_set_online(&(qualcomm_p_data->mhl_psy), true);
			power_supply_set_current_limit(&(qualcomm_p_data->mhl_psy),I_VBUS_PRE_DISCOVERY);
			power_supply_changed(&(qualcomm_p_data->mhl_psy));
			if (qualcomm_p_data->notify_usb_online){
				MHL_TX_DBG_WARN("qualcomm_p_data->notify_usb_online(qualcomm_p_data->notify_ctx, 1);  \n");
			...
			enable_intr(hw_context, INTR_DISC,
				...
				| BIT_CBUS_MHL12_DISCON_INT_MASK
				| BIT_RGND_READY_INT_MASK);
			//added by zhang yue on 2015-04-01 for fix problem start
			dev_context->mhl_mode = false;
			//added by zhang yue on 2015-04-01 for fix problem end
		}
		//added by zhang yue on 2015-04-01 for fix wait rgnd time out problem start 
		complete(&dev_context->rgnd_done);
		//added by zhang yue on 2015-04-01 for fix wait rgnd time out problem end 
		 ...
		 */
		printk(KERN_ERR "MHL connected\n\n");
		/*if(!wake_lock_active(&dev_context->mhl_on_wake_lock)){
			//wake_lock(&dev_context->mhl_on_wake_lock);
			printk(KERN_ERR "wake_lock(&dev_context->mhl_on_wake_lock);");
			}*/
		switch (hw_context->cbus_mode) {
		case CM_NO_CONNECTION:
	...
	struct interrupt_info *intr_info)
{
	...
	/*printk(KERN_ERR "si_mhl_tx_drv_device_isr() called.mhl_tx_read_reg(hw_context, REG_COC_STAT_0)=0x%x\n\n",mhl_tx_read_reg(hw_context, REG_COC_STAT_0));
...
void si_mhl_tx_drv_shutdown(struct drv_hw_context *hw_context)
{
	...
	//gpio_set_value(GPIO_BB_POWER_1V0, 0);
...
int si_mhl_tx_drv_connection_is_mhl3(struct mhl_dev_context *dev_context)
{
	struct drv_hw_context *hw_context =
	    (struct drv_hw_context *)&dev_context->drv_context;
	//printk(KERN_ERR "MHL connection mode is MHL3 :%d\n\n",IN_MHL3_MODE(hw_context));
				       ...
				       struct bist_setup_info *test_info)
{
	...
	case BIST_IMPEDANCE_MODE_ECBUS_S_TX_HIGH:
		...
		mhl_tx_write_reg(hw_context, REG_COC_CTL14, 0x00);
		mhl_tx_write_reg(hw_context, REG_COC_CTL15, 0x80);
		mhl_tx_write_reg(hw_context, REG_MHL_COC_CTL1, 0xBD);//mhl_tx_write_reg(hw_context, REG_MHL_COC_CTL1, 0x07);
		mhl_tx_write_reg(hw_context, REG_COC_CTL1, 0x10);
		mhl_tx_write_reg(hw_context, REG_COC_CTL2, 0x18);
		break;
