	...
	u32 event_param, void *data)
{
	...
	case MHL_TX_EVENT_RCP_RECEIVED:
		if (input_dev_rcp) {
			int result;
			pr_err("will call generate_rcp_input_event()");
			...
			snprintf(event_string, MAX_EVENT_STRING_LEN,
				...
				event_param);
			pr_err("MHLEVENT=received_RCP key code=0x%02x, input_dev_rcp=0",
				event_param);
			     ...
			     void (*usb_notify_cb)(void *, int), void *ctx)
{
	...
	if (!dev_context || !usb_notify_cb) {
		pr_err("%s: cb || dev_context is NULL\n", __func__);
	...
	//gpio_set_value(GPIO_BB_POWER_1V0, 1);
	...
	//set_pin(X02_USB_SW_CTRL, 1);
	pr_err("mhl  gpio_idsel level=%d  \n", gpio_get_value(GPIO_BB_ID_SEL));
	//gpio_direction_output(GPIO_BB_ID_SEL, 1);
	gpio_set_value(GPIO_BB_ID_SEL, 1);
	pr_err("mhl gpio_idsel level=%d @@@\n", gpio_get_value(GPIO_BB_ID_SEL));
	
	if(mhl_sii_wait_for_rgnd(dev_context) ){
		pr_err("%s: mhl_sii_wait_for_rgnd discovery timeout \n", __func__);
...
int mhl_tx_init(struct mhl_drv_info const *drv_info, struct device *parent_dev)
{
	...
	//wake_lock_init(&dev_context->mhl_on_wake_lock, WAKE_LOCK_SUSPEND, "mhl_on_lock");
	...
	if (dev_context->timer_work_queue == NULL) {
		pr_err("dev_context->timer_work_queue == NULL %s\n", __func__);
		ret = -ENOMEM;
		goto free_mem;
	}
	...
	if (mhl_class == NULL) {
		...
		if (IS_ERR(mhl_class)) {
			...
			pr_err("%s mhl class_create failed %d\n",__func__, ret);
			goto hid_cleanup;
		}
		...
		if (ret) {
			pr_err("register_chrdev %s failed, error code: %d\n",
				MHL_DRIVER_NAME, ret);
			goto free_class;
		}
	...
	if (IS_ERR(dev_context->mhl_dev)) {
		...
		pr_err("device_create failed %s %d\n", MHL_DEVICE_NAME, ret);
		goto free_cdev;
	}
	...
	if(ret){
       ...
       MHL_TX_DBG_INFO("MHL 8620 transmitter successfully initialized \n");
	return ret;
