...
static const struct hid_device_id mhl3_mt_devices[] = {
    ...
    { .driver_data = MT_CLS_SERIAL,
            HID_USB_DEVICE(USB_VENDOR_ID_SUO,
            USB_DEVICE_ID_SUO_HT70JF) },

    //SIMG GCS for Atmel
    //SIMG GCS for TPV
    { .driver_data = MT_CLS_DEFAULT,
    HID_USB_DEVICE(USB_VENDOR_ID_ATMEL,
        USB_DEVICE_ID_ATMEL_MXT2952T2) },

    /* SIMG GCS - 130603 - bz33633 */
    // XSENSE lacked a USB HID interface

    { .driver_data = MT_CLS_SERIAL,
            HID_USB_DEVICE(USB_VENDOR_ID_ATMEL,
                    USB_DEVICE_ID_ATMEL_TS_XSENSE) },

    { .driver_data = MT_CLS_OCULAR,
            HID_USB_DEVICE(USB_VENDOR_ID_ATMEL,
                    USB_DEVICE_ID_ATMEL_TS_MAXTOUCH) },

    //SIMG GCS for TPV; must be default
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_SIS,
                    USB_DEVICE_ID_SIS_TS_TPVDEMO) },


    //SIMG GCS for TPV; must be default
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_SIS,
                    USB_DEVICE_ID_SIS_SHANGHAI) },


    //SIMG GCS for TPV HA215AOM0
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_ELAN,
                    USB_DEVICE_ID_ELAN_HA215AOM0) },

    //SIMG GCS for TPV VS230AOS0
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_ELAN,
                    USB_DEVICE_ID_ELAN_VS230AOS0) },


    //SIMG GCS for TPV AO270WOM0
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_WEIDA,
                    USB_DEVICE_ID_WEIDA_AO270WOM0) },

    //SIMG GCS for TPV AO195WIM0
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_WEIDA,
                    USB_DEVICE_ID_WEIDA_AO195WIM0) },

    //SIMG GCS for TPV LN2116WxA0
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_WEIDA,
                    USB_DEVICE_ID_WEIDA_LN2116WxA0) },

    //SIMG GCS
    { .driver_data = MT_CLS_DEFAULT,
            HID_USB_DEVICE(USB_VENDOR_ID_SIS,
                    USB_DEVICE_ID_SIS_TS_LG23) },
