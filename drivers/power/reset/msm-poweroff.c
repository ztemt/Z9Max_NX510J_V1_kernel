...
static void msm_restart_prepare(const char *cmd)
{
 ...
 #ifdef CONFIG_ZTEMT_PANIC_BOOTMODE
  if (strncmp(cmd,"qcnreset",8) && strncmp(cmd,"updateimage",11) && (get_dload_mode() || (cmd != NULL && cmd[0] != '\0') || in_panic))
 #else
	if (strncmp(cmd,"qcnreset",8) && strncmp(cmd,"updateimage",11) && (get_dload_mode() || (cmd != NULL && cmd[0] != '\0')))
 #endif
		qpnp_pon_system_pwr_off(PON_POWER_OFF_WARM_RESET);
	else {
    if (strncmp(cmd,"qcnreset",8) && strncmp(cmd,"updateimage",11))
