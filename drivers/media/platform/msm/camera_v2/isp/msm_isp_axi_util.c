	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	if (stream_info->stream_type == BURST_STREAM) {
		if (stream_info->runtime_burst_frame_count == 0) {
			...
			// ZTEMT: fuyipeng 20150405 modify from qualcomm patch -----start
			framedrop_pattern =
      		           (1 << stream_info->runtime_num_burst_capture) - 1;
			framedrop_pattern <<= stream_info->runtime_init_frame_drop;
			//pr_err("fuyipeng --- framedrop_pattern :%d, runtime_init_frame_drop:%d \n", framedrop_pattern, stream_info->runtime_init_frame_drop);
		      // ZTEMT: fuyipeng 20150405 modify from qualcomm patch -----end
...
void msm_isp_reset_framedrop(struct vfe_device *vfe_dev,
	struct msm_vfe_axi_stream *stream_info)
{
      // ZTEMT: fuyipeng modify from qualcomm patch -----start
	...
	/*if (vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id == 0)
      ...
      // ZTEMT: fuyipeng modify from qualcomm patch -----end
