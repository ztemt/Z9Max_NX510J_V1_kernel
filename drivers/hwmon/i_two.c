...
#define IR_REMOTE_VDD_MAX_UV 3300000

static dev_t const i_two_ir_device_dev_t = MKDEV(MISC_MAJOR, 253);
...
static int i_two_power_init(struct i_two_chip *chip)
{
	int rc;
...
static void i_two_power_deinit(struct i_two_chip *chip)
{
	regulator_put(chip->power);
}

static int i_two_power_on(struct i_two_chip *chip, bool enable)
{
	int rc;
...
static void i_two_chip_data_init(struct i_two_chip *chip)
{
	chip->power_on = false;
}
...
{
	struct i_two_chip *chip = dev_get_drvdata(dev);
...
static struct device_attribute i_two_ir_attributes[] = {
...
static int create_sysfs_interfaces(struct i_two_chip *chip)
{
	int err = 0;
...
exit_device_attributes_create_failed:
	device_destroy(chip->ir_remote, i_two_ir_device_dev_t);
...
static void remove_sysfs_interfaces(struct i_two_chip *chip)
{
...
static int i_two_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	static struct i_two_chip *chip;
	int ret;

	SENSOR_LOG_INFO("prob start\n");

	chip = kzalloc(sizeof(struct i_two_chip), GFP_KERNEL);
	if (!chip) {
		ret = -ENOMEM;
		goto malloc_chip_failed;
	}

	i_two_chip_data_init(chip);
...
/*
 * i_two_remove() - remove device
 * @client: I2C client device
 */
static int i_two_remove(struct i2c_client *client)
{
	struct i_two_chip *chip = i2c_get_clientdata(client);

	i_two_power_deinit(chip);
	kfree(chip);
	chip = NULL;
	SENSOR_LOG_INFO("i_two_remove\n");
	return 0;
}

static int i_two_resume(struct device *dev)
{
	struct i_two_chip *chip = dev_get_drvdata(dev);

	SENSOR_LOG_INFO("enter\n");
	i_two_power_on(chip, true);
	SENSOR_LOG_INFO("eixt\n");

	return 0;
}

static int i_two_suspend(struct device *dev)
{
	struct i_two_chip *chip = dev_get_drvdata(dev);

	SENSOR_LOG_INFO("enter\n");
	i_two_power_on(chip, false);
	SENSOR_LOG_INFO("eixt\n");

	return 0;
}

static const struct dev_pm_ops i_two_pm_ops = {
	.suspend = i_two_suspend,
	.resume  = i_two_resume,
};

static const struct i2c_device_id i_two_idtable_id[] = {
	{"uie,i_two", 0},
	{},
};

static struct of_device_id of_i_two_idtable[] = {
	{.compatible = "uei,i_two",},
	{}
};

MODULE_DEVICE_TABLE(i2c, i_two_idtable);

static struct i2c_driver i_two_driver = {
	.driver = {
		.name = "i_two",
		.of_match_table = of_i_two_idtable,
		.pm = &i_two_pm_ops,
	},
	.id_table = i_two_idtable_id,
	.probe = i_two_probe,
	.remove = i_two_remove,
};

static int __init i_two_init(void)
{
	SENSOR_LOG_INFO("driver: init\n");
	return i2c_add_driver(&i_two_driver);
}

static void __exit i_two_exit(void)
{
	SENSOR_LOG_INFO("driver: exit\n");
	i2c_del_driver(&i_two_driver);
}

module_init(i_two_init);
module_exit(i_two_exit);

MODULE_DESCRIPTION("UEI i_two driver");
MODULE_LICENSE("GPL");
