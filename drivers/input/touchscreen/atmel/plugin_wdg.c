	...
	if (test_flag(WD_FLAG_REG_OVERFLOW ,&flag)) {
		...
		if (re_obs->curr >= re_cfg->failed_reset_count) {
			if (re_obs->retry < re_cfg->failed_reset_retry) {
				dev_info2(dev, "mxt WD reg set reset count %d(%d) retry %d(%d)\n", 
...
static long wdg_handle_ti_event(struct plugin_wdg *p, unsigned long flag)
{
	...
	if(test_flag(WD_FLAG_RESET ,&flag)) {
		...
		dev_info2(dev, "WD hook T61 check overflow\n");
...
static void plugin_wdg_hook_t6(struct plugin_wdg *p, u8 status)
{
	...
	dev_info2(dev, "mxt wd flag=0x%lx\n",
		 obs->flag);
}
...
static int plugin_wdg_wd_init(struct plugin_wdg *p)
{
	const struct mxt_config *dcfg = p->dcfg;
	struct device *dev = dcfg->dev;
...
static void plugin_wdg_wd_deinit(struct plugin_wdg *p)
{
	deinit_wd_object(p);
...
struct plugin_wdg mxt_plugin_wdg = 
{
	.init = plugin_wdg_wd_init,
	.deinit = plugin_wdg_wd_deinit,
...
int plugin_wdg_init(struct plugin_wdg *p)
{
	memcpy(p, &mxt_plugin_wdg, sizeof(struct plugin_wdg));
